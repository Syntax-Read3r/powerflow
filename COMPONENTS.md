...
| `adapters/locations.ps1` | `Get-StarshipConfigPath`, `Get-TerminalSettingsPath`, `Get-TempPath`, `Get-HomePath`, `Get-PowerFlowDataPath`, `Get-PowerFlowConfigPath`, `Get-PowerFlowNavigationDataPath`, `Get-UserFolderPath` | `%LOCALAPPDATA%`, `%TEMP%`; `Get-PowerFlowNavigationDataPath` honours `$env:POWERFLOW_DATA_HOME` and otherwise keeps the legacy home path; **`Get-UserFolderPath` reads the Known Folder registry** — `Join-Path $HOME 'Documents'` is WRONG once OneDrive Known Folder Move redirects Documents/Pictures/Desktop to `~\OneDrive\…`, leaving the local path an empty stub or absent (measured: `~\Pictures` did not exist at all). Takes `-Prefer auto\|local\|known` so someone who keeps files off OneDrive can say so | XDG (`~/.config`, `~/.local/share`), `$TMPDIR`; the navigation-data override has the same contract and fallback; **`Get-UserFolderPath`** faces the same trap in another shape — XDG user dirs can be relocated or localised (`~/Documentos`) — so it uses `xdg-user-dir(1)`, then parses `~/.config/user-dirs.dirs`, then falls back to `~/<Name>` |
...
| `adapters/perms.ps1` | `Get-FileMode`, `Set-FileMode`, `Test-PermsSupported`, `Get-Umask`, `Set-Umask` | **returns `$null`** — Windows has ACLs, not POSIX mode bits, and inventing a fake `755` would teach something false | `stat(1)` for the mode; **libc `umask(2)` via P/Invoke** for the umask³ |
...
| `adapters/sysconfig.ps1` | `Test-SysConfigSupported`, `Get-SysConfigOptions`, `Get-SysConfigChoices`, `Set-SysConfig`, `Test-HostNameValid`, `Get-HostRenamePlan`, `Set-HostRename`, `Test-HostResolution` — plus Linux-internal `Get-CurrentHostName`, `Set-MachineHostName` | `Set-TimeZone` / `Set-Culture` / `Rename-Computer` / W32Time, each applied for the user — machine-wide ones via a single UAC prompt (`Invoke-ElevatedCommand`). Four settings: keyboard is omitted because a Windows layout is an input-language tip, not a keymap. Validates a value against its own choices first (`Set-Culture` will otherwise write a bogus culture). **Rename:** `Rename-Computer`, reported as taking effect on restart; there is no `/etc/hosts` analogue to sync, so that half is absent rather than faked | `localectl` / `timedatectl` / `hostnamectl`; a domain model (keyboard/timezone/locale/hostname/ntp) so new settings are one row. Keyboard detects vconsole vs X11 (`Get-KeyboardMode`) so Debian/Ubuntu work too. **Rename:** previews the `/etc/hosts` edit before applying it, backs the file up, and rewrites **only** the 127.x line naming this host. Falls back to `hostname` + `/etc/hostname` where there is no systemd (Alpine, Arch). Rewritten in-process rather than with `sed -i` — .NET regex escaping and POSIX BRE disagree on exactly the characters that matter |
...
| `components/shared/volumes.ps1` | Both | `Test-PFPathWritable`, `Get-PFStorageCandidate` — which volumes could hold what grows, and **why the others could not**¹⁵ |
| `components/shared/outcome.ps1` | Both | `Write-PFCancelled`, `Write-PFFailure`, `Write-PFWarning`, `Write-PFNothingFound`, `Invoke-PFNativeStep` — **the message names the outcome that actually happened, the marker names its severity, and both are read from a signal the code is holding.** ❌ is for a failure or a refused mutation; ↩ dim for a user decision; ⭕ for a query's truthful negative. Spend the red marker on someone who merely changed their mind and they learn to scan past it. `Invoke-PFNativeStep` captures `$LASTEXITCODE` on the line after the call and prints no success message of its own — what success means differs per command¹⁶ |
| `components/shared/timerange.ps1` | Both | `Resolve-PFDayToken`, `ConvertTo-PFTimeRange`, `Add-PFClockTime` — `t@00:40 to 00:50` resolved to two exact instants. Shared because PF-FEAT-004 wants it on `pman events` and PF-FEAT-005 on `pman logs`. **Never rolls past midnight silently**, **never guesses between 05/03 and 03/05** (culture order, flagged for the caller to echo), and **each end carries the offset for its own date** — resolving yesterday with today's offset is an hour wrong across a DST change. `$Now` is injectable so the suite is not hostage to the clock¹⁷ |
| `components/shell/bash-compat.ps1` | Shell | `export`, `unset`, `source`, `alias`¹, `unalias`, `jobs`, `fg`, `bg` — the bash builtins PowerShell lacks |
| `components/shell/keys.ps1` | Both | `Get-PFEditingChords`, `ConvertTo-PFChordKey`, `Get-PFKeyBindingState`, `Set-PFEditingKeys`, `Show-PFKeyBindings`, `pwsh-keys` — word navigation/deletion/selection chords. **Additive only:** binds what is UNBOUND, never replaces a working binding (`--rebind` is the one explicit override). Emacs mode — the Linux default — leaves five of these unbound, which is the actual defect behind PF-UX-003; on Windows they were already bound, so that half is terminal delivery, which `pwsh-keys` says plainly rather than papering over with a remap. `ConvertTo-PFChordKey` exists because **PSReadLine normalises modifier order when reporting** (`Ctrl+Shift+X` → `Shift+Ctrl+X`), so a raw lookup reports a bound chord as unbound and then double-binds it | Same on both |
...
>
> ¹² **`pc-name` changes two things because changing one is the bug.** `hostnamectl
> set-hostname web-prod` leaves `/etc/hosts` naming the old host, and the next `sudo` prints
> *"unable to resolve host web-prod"* — it still works, after a fallback timeout, so the cost
> shows up as every elevated command being slower rather than as an error anyone traces. So
> the rename previews **both** edits, backs `/etc/hosts` up, rewrites only the 127.x line that
> already names this host — an operator's own static entries are not a rename's business — and
> then *verifies* the new name resolves rather than assuming it. If the hosts edit fails after
> the rename succeeded it says so exactly, and hands over the command to finish: a partial
> change reported as a failure would send someone looking for a rename that already happened.
...
> ¹⁵ **Drive type is not a safety signal, and one rule about hardware may not exist twice.**
> Windows reports a USB-attached external disk as `DriveType='Fixed'` — measured on a WD My
> Passport showing 481 GB free and `IsSystem` false, which is indistinguishable from an ideal
> second drive to any check based on `IsSystem` alone. Eligibility therefore joins against
> `Get-DiskInfo`, because the bus lives on the DISK; its `Letters` field is space-joined and
> holds drive letters on Windows and mount points on Linux, which is why a single join serves
> both platforms. Writability is **probed** with a create-and-delete, never inferred: a mount
> can be read-only or owned by another user while its permission bits look fine.
>
> It lives in `shared/` rather than beside `storage` because **`nav setup` asks the same
> question of directories that `storage root` asks of volumes**, and each had grown its own
> copy — the exact drift that put a hand-maintained CI list five names behind reality. It
> cannot live in `components/system/` either: the bootloader loads `navigation` FIRST, so the
> dependency would run backwards through the load order and trap whoever next reorders it.
...
| `components/files/rename.ps1` | Both | `rn`¹¹ (shim) + `Invoke-PFRenameFile` — interactive rename; `--chmod <mode>` applies a POSIX mode to the **new** path afterwards and verifies it. A failed chmod never rolls the rename back: it reports the partial success and prints the exact recovery command. Linux only — Windows refuses rather than mapping NTFS ACLs to a numeric mode | `rn` |
...
| `components/network/servers.ps1` | Network | `srv` (private alias/status picker: Enter connect · ctrl-d delete · ctrl-r rename), `srv <name> info`, `srv add`, `srv rm`, `srv rename`, `srv list`, `Show-PFServerPicker`, `Test-ServerOnline`, `Get-PFHostReachability`, `Get-PFServers`, `Save-PFServers` — named SSH connections in `~/.powerflow-servers.json`. Status = TCP probe of the ssh port with ICMP tiebreaker: `online` / `no-ssh` / `offline`. `Get-PFHostReachability` is the parallel probe, shared with PMX's `pmx net status` — its body is inlined rather than calling `Test-ServerOnline` because a `ForEach-Object -Parallel` runspace cannot see local functions; the duplication is forced by that boundary, and keeping it to one copy is why it lives here. `ssh` itself is never shadowed. |
