...
| `components/system/health.ps1` | System | `pc-whoami` (+ `-power`, `-crashes [-export]`, `-bios`, `-ram [-min N]`, `-days N`), `Show-RamDetail`, `Stop-RamHog`, `Format-DriveSize`, `pc-cap`⁷ — machine vitals + a CPU cap with guaranteed restoration. Design: [docs/plan/pc-whoami/](docs/plan/pc-whoami/README.md) |
