...
| `components/system/health.ps1` | System | `pc-whoami` (+ `-power`, `-crashes [-export]`, `-bios`, `-ram [-min N]`, `-ram <name>`, `-days N`), `Show-RamDetail` (read-only overview), `Show-RamProcesses` (drill-in with command lines), `Stop-RamProcess` (one PID), `Stop-RamGroup` (whole program, harder warning), `Format-DriveSize`, `pc-cap`⁷ — machine vitals + a CPU cap with guaranteed restoration. Design: [docs/plan/pc-whoami/](docs/plan/pc-whoami/README.md) |
