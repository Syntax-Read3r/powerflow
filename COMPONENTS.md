...
| `adapters/sysconfig.ps1` | `Test-SysConfigSupported`, `Get-SysConfigOptions`, `Get-SysConfigChoices`, `Set-SysConfig` | **all no-op/`$false`** — no systemd on Windows; settings live in Windows Settings / `Set-TimeZone` / `Rename-Computer` | `localectl` / `timedatectl` / `hostnamectl`; a domain model (keyboard/timezone/locale/hostname/ntp) so new settings are one row |
...
| `components/system/sysconfig.ps1` | System | `pwsh-config` (Linux), `Complete-SysConfigChange` — one fzf-driven menu to change OS settings (keyboard/timezone/locale/hostname/time-sync) via systemd, replacing the Debian-only `dpkg-reconfigure`. Extensible: new settings are a row in the adapter |
