...
| `components/system/login.ps1` | System | `pwsh-autologin` (Linux) — toggle "start PowerFlow on login" without re-running the installer; writes the same guarded hook as `install.sh --auto-login` |
