# ==============================================================================
# PowerFlow — Proxmox Management Adapter (Linux)
# ==============================================================================
# Domain   : Platform / Linux
# File     : platform/linux/adapters/proxmox-management.ps1
# Purpose  : Allow-listed local and SSH Proxmox VM-management operations
...
function Test-PmxManagementRoot {
    if (-not (Test-Path -LiteralPath '/proc/self/status')) { return $false }
    $uid = @(Get-Content -LiteralPath '/proc/self/status' -ErrorAction SilentlyContinue |
        Where-Object { $_ -match '^Uid:\s+' } | Select-Object -First 1)
    return ($uid.Count -eq 1 -and "$($uid[0])" -match '^Uid:\s+0\s')
}
...
function Resolve-PmxManagementAdapterConnection {
    ...
    if ([string]::Equals($transport, 'local', [StringComparison]::Ordinal)) {
        if (-not (Test-Path -LiteralPath '/etc/pve')) {
            return [pscustomobject]@{ Success = $false; Data = $null; Error = 'Local transport requires a Proxmox VE node.' }
        }
        if (-not (Test-PmxManagementRoot)) {
            return [pscustomobject]@{ Success = $false; Data = $null; Error = 'Local Proxmox management requires root.' }
        }
        $pvesh = Get-Command 'pvesh' -CommandType Application -ErrorAction SilentlyContinue | Select-Object -First 1
        $qm = Get-Command 'qm' -CommandType Application -ErrorAction SilentlyContinue | Select-Object -First 1
        if (-not $pvesh -or -not $qm) {
            return [pscustomobject]@{ Success = $false; Data = $null; Error = 'Local transport requires the pvesh and qm applications.' }
        }
        return [pscustomobject]@{
            Success = $true
            Error   = ''
