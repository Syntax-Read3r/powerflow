# ==============================================================================
# PowerFlow — Container Engine Adapter (Windows)
# ==============================================================================
# Domain   : Platform / Windows
# File     : platform/windows/adapters/container.ps1
...
# ==============================================================================
#
# HOW THIS DIFFERS FROM THE LINUX ADAPTER
#
# The CLI surface is identical, so parsing and command construction match line for line.
# Elevation does not exist here at all:
#
#   Docker Desktop talks over the npipe://./pipe/docker_engine named pipe, whose ACL is
#   granted to the docker-users group at install time. There is no per-invocation
#   elevation — either your account is in that group or it is not.
#
#   Podman on Windows runs inside a WSL machine. There is no host-side socket to have
#   permissions on, and no sudo. "Installed but unreachable" almost always means the
#   machine is stopped, which is a `podman machine start` away rather than a fault.
#
# So NeedsSudo is always $false here, for both engines.
#
# The `{{json .}}` divergence and the tab-template reasoning are documented in the Linux
# adapter and apply identically — podman's JSON uses `Id` not `ID`, and returns Names,
# Labels and Ports as array/object/array where docker returns strings. Measured against
# podman 6.0.2 on this host, not assumed.
# ==============================================================================

# Field order is load-bearing: it must match the split in Get-ContainerList, and Labels
# must stay last so a tab inside a label value cannot shift the other columns.
...
.DESCRIPTION
    Same four states as Linux so the component never branches on OS:
      missing      the CLI is not installed
      unreachable  CLI present, engine not up (Desktop not started / machine stopped)
      needs-sudo   reported for a pipe-ACL refusal; never involves elevation on Windows
      ready        usable as-is
#>
function Get-ContainerEngineInfo {
    ...
    if (-not $version) {
        ...
        # A pipe-ACL refusal is a group-membership problem, which is fixable but NOT by
        # elevating a single command — so it is reported without ever setting NeedsSudo.
        if ($Engine -eq 'docker' -and $probe -match 'Access is denied|denied the operation') {
            $descriptor.State = 'needs-sudo'
            $descriptor.Error = 'this account cannot open the Docker named pipe'
            return $descriptor
        }
    ...
    # Podman delegates compose to an external provider and announces it on every call.
...
# NeedsSudo is honoured in the signature but never true on Windows, so the component's
# code path is identical across platforms.
...
# observed on this host: ESC[4m>>>> Executing external compose provider
# "…docker-compose.exe" … <<<< then ESC[0m[]. Left in place the JSON is unparseable and the
# failure is SILENT, because the leading byte simply stops being '['.
...
function Get-ContainerLogCommand {
    ...
    return [pscustomobject]@{ File = $Engine.Binary; Arguments = $engineArgs
        Native = "$($Engine.Binary) " + ($engineArgs -join ' ') }
}

# Most self-hosted images are Alpine-based and ship no bash, so it is probed rather than
# assumed.
...
<#
.SYNOPSIS
    Bring a compose project up or down. `down` is never given -v.
#>
