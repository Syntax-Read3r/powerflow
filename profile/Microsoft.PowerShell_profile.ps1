# ============================================================================
# PowerShell Profile for Munya 
# ============================================================================
# A comprehensive PowerShell profile with navigation, Git utilities, terminal
# management, and productivity shortcuts. All functions documented and optimized.
# ============================================================================
...
#     if (-not $fileName) {
#         Write-Host "💡 Smart Remove Commands:" -ForegroundColor Cyan
#         Write-Host "═══════════════════════" -ForegroundColor Cyan
#         Write-Host "  rm <filename>          Remove file (smart search)" -ForegroundColor DarkGray
#         Write-Host "  rm <filename> -Force   Remove without confirmation" -ForegroundColor DarkGray
#         Write-Host "  rm <filename> -Recurse Remove directory recursively" -ForegroundColor DarkGray
#         Write-Host "  rm <filename> -verbose Show detailed search process" -ForegroundColor DarkGray
#         return
#     }
    
#     if ($verbose) {
#         Write-Host "=== SMART RM FUNCTION ===" -ForegroundColor Cyan
#         Write-Host "Searching for: '$fileName'" -ForegroundColor Yellow
#         Write-Host "Current directory: $PWD" -ForegroundColor Yellow
#     }
...
#     # Handle special cases
#     if ($fileName -eq "." -or $fileName -eq "..") {
#         Write-Host "❌ Cannot remove current or parent directory reference" -ForegroundColor Red
#         return
#     }
    
#     # Try exact path first (absolute or relative)
#     if (Test-Path $fileName) {
#         if ($verbose) { Write-Host "✅ Found exact path: $fileName" -ForegroundColor Green }
#         Remove-Item $fileName -Recurse:$Recurse -Force:$Force
#         Write-Host "🗑️ Removed: $fileName" -ForegroundColor Green
#         return
#     }
    
#     # === SMART SEARCH LOGIC (like nav function) ===
    
#     if ($verbose) { Write-Host "`n🔍 Starting smart search in current directory..." -ForegroundColor Cyan }
...
#         # Phase 1: Look for EXACT MATCHES
#         if ($verbose) { Write-Host "`n📋 Phase 1: Checking for exact matches..." -ForegroundColor Magenta }
        
#         $exactMatches = @()
#         foreach ($item in $allItems) {
#             if ($item.Name -eq $fileName) {
#                 $exactMatches += $item
#                 if ($verbose) { Write-Host "  ⭐ EXACT MATCH: $($item.Name)" -ForegroundColor Green }
#             }
#         }
        
#         if ($exactMatches.Count -eq 1) {
#             $targetItem = $exactMatches[0]
#             Remove-Item $targetItem.FullName -Recurse:$Recurse -Force:$Force
#             Write-Host "🗑️ Removed exact match: $($targetItem.Name)" -ForegroundColor Green
#             return
#         } elseif ($exactMatches.Count -gt 1) {
#             Write-Host "⚠️ Multiple exact matches found:" -ForegroundColor Yellow
#             for ($i = 0; $i -lt $exactMatches.Count; $i++) {
#                 Write-Host "  [$($i+1)] $($exactMatches[$i].Name) ($($exactMatches[$i].GetType().Name))" -ForegroundColor Cyan
#             }
#             $choice = Read-Host "Enter number to remove (or 'q' to quit)"
#             if ($choice -eq 'q') { return }
#             if ([int]::TryParse($choice, [ref]$null) -and $choice -ge 1 -and $choice -le $exactMatches.Count) {
#                 $targetItem = $exactMatches[$choice - 1]
#                 Remove-Item $targetItem.FullName -Recurse:$Recurse -Force:$Force
#                 Write-Host "🗑️ Removed: $($targetItem.Name)" -ForegroundColor Green
#                 return
#             } else {
#                 Write-Host "❌ Invalid selection" -ForegroundColor Red
#                 return
...
#         # Phase 2: Look for FUZZY MATCHES (contains the search term)
#         if ($verbose) { Write-Host "`n📋 Phase 2: Checking for fuzzy matches..." -ForegroundColor Magenta }
        
#         $fuzzyMatches = @()
#         foreach ($item in $allItems) {
#             if ($item.Name -like "*$fileName*" -and $item.Name -ne $fileName) {
#                 $fuzzyMatches += $item
#                 if ($verbose) { Write-Host "  ⚡ FUZZY MATCH: $($item.Name)" -ForegroundColor Yellow }
#             }
#         }
...
#         # Phase 3: Try common file extensions
#         if ($verbose) { Write-Host "`n📋 Phase 3: Trying common file extensions..." -ForegroundColor Magenta }
...
#             if ($match) {
#                 $extensionMatches += $match
#                 if ($verbose) { Write-Host "  💡 EXTENSION MATCH: $testName" -ForegroundColor Cyan }
#             }
#         }
...
#     Write-Host "   • Partial filename matches (fuzzy)" -ForegroundColor DarkGray
#     Write-Host "   • Common file extensions (.txt, .md, .json, etc.)" -ForegroundColor DarkGray
#     Write-Host "💡 Use 'rm $fileName -verbose' for detailed search output" -ForegroundColor DarkGray
#     Write-Host "💡 Use full filename if you know it exactly" -ForegroundColor DarkGray
# }
# NOTE: PROTECTED CODE ABOVE
...
#         [switch]$Recurse,
#         [switch]$Force,
#         [switch]$f,  # Short alias for Force (remove [Alias("f")] line)
#         [switch]$verbose
#     )
...
#         Write-Host "═══════════════════════" -ForegroundColor Cyan
#         Write-Host "  rm <filename>          Remove file (smart search)" -ForegroundColor DarkGray
#         Write-Host "  rm <filename> -f       Force remove (hidden files, .git, etc.)" -ForegroundColor DarkGray
#         Write-Host "  rm <filename> -Force   Same as -f" -ForegroundColor DarkGray
#         Write-Host "  rm <filename> -Recurse Remove directory recursively" -ForegroundColor DarkGray
#         Write-Host "  rm <filename> -verbose Show detailed search process" -ForegroundColor DarkGray
#         Write-Host "  rm -f <folder>         Force remove hidden/protected folders" -ForegroundColor DarkGray
#         return
#     }
    
#     # Combine -f and -Force flags
#     $forceRemoval = $Force -or $f
    
#     if ($verbose) {
#         Write-Host "=== SMART RM FUNCTION ===" -ForegroundColor Cyan
#         Write-Host "Searching for: '$fileName'" -ForegroundColor Yellow
#         Write-Host "Current directory: $PWD" -ForegroundColor Yellow
#         Write-Host "Force removal: $forceRemoval" -ForegroundColor Yellow
#         Write-Host "Include hidden files: $($forceRemoval -or $fileName -like ".*")" -ForegroundColor Yellow
#     }
...
#         # Special handling for .git directories and hidden files
#         if ($fileName -like "*.git*" -or $fileName -like ".*") {
#             if ($forceRemoval) {
#                 Remove-Item $fileName -Recurse:$true -Force:$true -ErrorAction SilentlyContinue
#                 Write-Host "🗑️ Force removed: $fileName" -ForegroundColor Green
#             } else {
#                 Write-Host "⚠️ Hidden/protected file detected. Use -f to force remove: $fileName" -ForegroundColor Yellow
#                 return
#             }
#         } else {
#             Remove-Item $fileName -Recurse:$Recurse -Force:$forceRemoval
#             Write-Host "🗑️ Removed: $fileName" -ForegroundColor Green
#         }
#         return
#     }
...
#     try {
#         # Get all items in current directory (include hidden if force or searching for hidden)
#         $includeHidden = $forceRemoval -or $fileName -like ".*" -or $fileName -like "*.git*"
#         $allItems = Get-ChildItem -Path $currentPath -Force:$includeHidden -ErrorAction SilentlyContinue
        
#         if ($verbose) {
#             Write-Host "Found $($allItems.Count) items in current directory (hidden: $includeHidden)" -ForegroundColor Yellow
#         }
...
#             # Special handling for hidden/protected files
#             if ($targetItem.Name -like ".*" -or $targetItem.Name -like "*.git*") {
#                 if ($forceRemoval) {
#                     Remove-Item $targetItem.FullName -Recurse:$true -Force:$true -ErrorAction SilentlyContinue
#                     Write-Host "🗑️ Force removed exact match: $($targetItem.Name)" -ForegroundColor Green
#                 } else {
#                     Write-Host "⚠️ Hidden/protected file detected. Use -f to force remove: $($targetItem.Name)" -ForegroundColor Yellow
#                     return
#                 }
#             } else {
#                 Remove-Item $targetItem.FullName -Recurse:$Recurse -Force:$forceRemoval
#                 Write-Host "🗑️ Removed exact match: $($targetItem.Name)" -ForegroundColor Green
#             }
#             return
#         } elseif ($exactMatches.Count -gt 1) {
...
#                     if ($forceRemoval) {
#                         Remove-Item $targetItem.FullName -Recurse:$true -Force:$true -ErrorAction SilentlyContinue
#                         Write-Host "🗑️ Force removed: $($targetItem.Name)" -ForegroundColor Green
#                     } else {
#                         Write-Host "⚠️ Hidden/protected file detected. Use -f to force remove: $($targetItem.Name)" -ForegroundColor Yellow
#                         return
#                     }
...
#         # Phase 3: Try common file extensions (including hidden file patterns)
#         if ($verbose) { Write-Host "`n📋 Phase 3: Trying common file extensions..." -ForegroundColor Magenta }
        
#         $commonExtensions = @(".txt", ".md", ".json", ".xml", ".csv", ".log", ".ps1", ".py", ".js", ".html", ".css", ".gitignore", ".gitattributes")
#         $extensionMatches = @()
        
#         # Also try hidden file patterns
#         $hiddenPatterns = @(".$fileName")
#         if ($fileName -eq "git") {
#             $hiddenPatterns += @(".git", ".gitignore", ".gitattributes", ".gitmodules")
#         }
...
#         # Check hidden patterns if force is enabled or searching for hidden files
#         if ($includeHidden) {
#             foreach ($pattern in $hiddenPatterns) {
#                 $match = $allItems | Where-Object { $_.Name -eq $pattern }
#                 if ($match) {
#                     $extensionMatches += $match
#                     if ($verbose) { Write-Host "  🔒 HIDDEN MATCH: $pattern" -ForegroundColor Cyan }
#                 }
#             }
#         }
...
#     Write-Host "   • Common file extensions (.txt, .md, .json, etc.)" -ForegroundColor DarkGray
#     if ($includeHidden) {
#         Write-Host "   • Hidden files and .git patterns" -ForegroundColor DarkGray
#     } else {
#         Write-Host "💡 Use 'rm $fileName -f' to search hidden files (.git, etc.)" -ForegroundColor Yellow
#     }
#     Write-Host "💡 Use 'rm $fileName -verbose' for detailed search output" -ForegroundColor DarkGray
#     Write-Host "💡 Use full filename if you know it exactly" -ForegroundColor DarkGray
# }

# protected code above
...
# function gh-l {
#     param (
#         [string]$Sort = "pushed",  # pushed, updated, created, full_name
#         [int]$Count = 10,
#         [string]$Token,
#         [switch]$SaveToken
#     )
...
#     $tokenPath = Join-Path $env:APPDATA "gh-l-token.txt"
    
#     # Get token from various sources
#     if (-not $Token) {
#         # Try parameter, then environment, then saved file
...
#                 )
#                 Write-Host "🔐 Using saved token" -ForegroundColor DarkGreen
#             } catch {
#                 Write-Host "⚠️ Could not read saved token, it may be corrupted" -ForegroundColor Yellow
...
#         $secureInput = Read-Host "🔑 Enter your GitHub token (hidden)" -AsSecureString
#         $Token = [System.Runtime.InteropServices.Marshal]::PtrToStringAuto(
#             [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($secureInput)
#         )
        
#         if ($Token) {
#             $save = Read-Host "💾 Save token securely for future use? (y/n)"
#             if ($save -eq 'y') {
#                 try {
#                     $secureInput | ConvertFrom-SecureString | Out-File $tokenPath
...
#     # Use authenticated user repos endpoint (includes private repos)
#     # Sort by 'pushed' to get most recently pushed repos first
#     $url = "https://api.github.com/user/repos?per_page=$Count&sort=$Sort&direction=desc&affiliation=owner"
...
#         Write-Host "🔐 Fetching your most recently $Sort repositories..." -ForegroundColor Cyan
...
#         Write-Host "📦 Showing top $Count repositories (sorted by most recent $Sort)" -ForegroundColor Cyan
#         Write-Host "💡 Use fzf search (type to filter) or try different -Sort/-Count for more" -ForegroundColor DarkGray
        
#         $choices = $repos | ForEach-Object {
#             $privacy = if ($_.private) { "🔒" } else { "🌐" }
#             $language = if ($_.language) { "[$($_.language)]" } else { "[No language]" }
            
#             # Use pushed_at for most recent activity, fallback to updated_at
#             $dateField = if ($Sort -eq "pushed" -and $_.pushed_at) { $_.pushed_at } else { $_.updated_at }
#             $formattedDate = ([DateTime]$dateField).ToString("yyyy-MM-dd")
            
#             "{0,-35}  {1} ⭐ {2,-5}  🍴 {3,-5}  📅 {4,-10}  {5,-12}  🌐 {6}" -f `
#                 $_.name, $privacy, $_.stargazers_count, $_.forks_count, `
#                 $formattedDate, $language, $_.html_url
#         }
        
#         $selection = $choices | fzf --ansi --reverse --height=80% --border `
#             --prompt="📦 Your Repos ($Count shown, recent $Sort): " --header="⬇️ Select a repo | Type to search | Enter: Copy URL | 🔒=Private 🌐=Public"
        
#         if ($selection -match 'https://github.com/\S+') {
#             $repoUrl = $matches[0]
#             Set-Clipboard $repoUrl
#             Write-Host "📋 Copied URL: $repoUrl" -ForegroundColor Green
#             Write-Host "`n🔧 What would you like to do with this repo?" -ForegroundColor Cyan
#             Write-Host "  1. Clone" -ForegroundColor DarkGray
#             Write-Host "  2. Open in browser" -ForegroundColor DarkGray
#             Write-Host "  3. Copy SSH URL instead" -ForegroundColor DarkGray
#             Write-Host "  4. Just copied HTTP URL" -ForegroundColor DarkGray
...
#                 $remove = Read-Host "🗑️ Remove saved token? (y/n)"
#                 if ($remove -eq 'y') {
#                     Remove-Item $tokenPath
#                     Write-Host "🗑️ Saved token removed" -ForegroundColor Yellow
#                 }
#             }
...
# # Helper function to remove saved token
# function gh-l-reset {
#     $tokenPath = Join-Path $env:APPDATA "gh-l-token.txt"
#     if (Test-Path $tokenPath) {
#         Remove-Item $tokenPath
#         Write-Host "🗑️ Saved GitHub token removed" -ForegroundColor Green
#     } else {
#         Write-Host "ℹ️ No saved token found" -ForegroundColor Yellow
#     }
# }
# protect code above
...
Write-Host "✅ PowerShell profile loaded! Type " -NoNewline -ForegroundColor Green
