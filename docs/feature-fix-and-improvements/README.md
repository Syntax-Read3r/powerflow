...
| PF-UX-001 | UX | `pmx list` / `pmx status` + typo suggestions | **done** — both routes call the canonical function; suggestions are derived from the help catalogue (never a second list), prefix-first, bounded to 3, and only ever printed. A test cross-checks every suggestible route against the router. `tests/proxmox/route-suggestions.ps1` |
| PF-FEAT-008 | FEATURE | `pmx net` fleet view with SSH reachability | **done** — `pmx net status` (fleet) and `pmx net <vm> status`. Seven distinct SSH states, never flattened; probes only agent-reported addresses and never scans (asserted by standing in for the prober); `ready` = TCP only, stated in the view and the JSON. Reuses the existing network model and `srv`'s probe, extracted as `Get-PFHostReachability`. `tests/proxmox/fleet-network-status.ps1` |
| PF-FEAT-005 | FEATURE | `pman`/`dkr` logs refinement + readable `inspect` | **mostly done** — timestamps + 30-line default, conservative tidying with a never-collapse list, `--raw`/`-a`/`--tail N`/positional tail, untouched `-f` streaming, lifecycle footer, and `pman inspect`/`show` (+`--json`). **Deferred:** the `t@HH:MM` time-range grammar, which the item itself asks to be shared with PF-FEAT-004 rather than duplicated. `tests/containers/log-view.ps1` |
| PF-UX-003 | INVESTIGATE | word navigation / selection keys | **done** — investigated, and it split in two. PowerFlow was not breaking anything, and on Windows every reported chord was already bound; the real gap is Emacs mode (the Linux default), where five are unbound. Filled in additively, never replacing a bound chord. `pwsh-keys` reports what is bound and by whom, and names the terminal-delivery case no remap can fix. `tests/shell/editing-keys.ps1` |
